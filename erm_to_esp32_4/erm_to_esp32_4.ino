...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(750);  // LED BLINKS WHEN TRYING TO CONNECT
    if (blink) {
...
void processRequest(WiFiClient& client, String requestStr) {
  ...
  if (requestStr.startsWith("GET /toggle")) {
    ...
    client.println("<h3>Missing Particletron Trigger.</h3>");
    client.println("</body>");
    client.println("</html>");
  }
}
