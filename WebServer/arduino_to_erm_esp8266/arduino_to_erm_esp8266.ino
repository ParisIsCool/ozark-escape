...
const char* ssid = "Hitron11330-EasyConnect";
const char* password = "D6DFBHitron";

// Set your Static IP address
// This is what is used to connect to it in ERM
// IMPORTANT: This must be unique for each arduino!
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(250);  // LED BLINKS WHEN TRYING TO CONNECT
    if (blink) {
...
void CheckPin(WiFiClient& client, int pin) {
  String status = (digitalRead(pin) == HIGH) ? "triggered" : "not triggered";
  status = (testPin == true) ? "triggered" : status; // IF TESTPIN TRIGGERED, CHANGE, ELSE, GO TO READ FROM ABOVE.
...
void processRequest(WiFiClient& client, String requestStr) {
  ...
  if (requestStr.startsWith("GET /1")) {
    CheckPin(client, PIN_1);
  } else if (requestStr.startsWith("GET /2")) {
    CheckPin(client, PIN_2);
  } else if (requestStr.startsWith("GET /3")) {
    CheckPin(client, PIN_3);
  } else if (requestStr.startsWith("GET /4")) {
    CheckPin(client, PIN_4);
...
void loop() {
  ...
  // COMMAND TO TEST IF TRIGGER WORKS
